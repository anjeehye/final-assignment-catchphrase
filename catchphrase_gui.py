...
You can customize the design (color, font, etc.) of the GUI by playing with
the first part of the script under the comment 'GUI settings'.

This script was inspired by: https://youtu.be/A0gaXfM1UN0
"""
...
# GUI settings: color scheme, max image size, font
COLORS = {
    'background': '#232323',
    'highlight': '#48e9d0',
    }
IMG_MAX_DIMENSION = 600  # max image width/height (in pixels)
FONT = 'courier'
...
class catchphrase_gui(tk.Tk):
    ...
    def image_rescale(self, images, max_dimension):
        # if either the width/height of the image is greater than the
        ...
        if (width > max_dimension) or (height > max_dimension):
            ...
            for i in range(0, len(images)):
                images[i] = st.rescale(images[i], factor, multichannel=colored)
...
class StartPage(tk.Frame):
    def __init__(self, parent, controller, images, reveal_number):
        ...
        startpage_frame = tk.Frame(self, bg=COLORS['background'])
...
class GamePage(tk.Frame):
    def __init__(self, parent, controller, images, reveal_number):
        ...
        gamepage_frame = tk.Frame(self, bg=COLORS['background'])
...
class FinalReveal(tk.Frame):  # Reveal the original image
    def __init__(self, parent, controller, images, reveal_number):
        ...
        final_reveal_frame = tk.Frame(self, bg=COLORS['background'])
...
def play_catchphrase_gui(filename, reveal_number=3):
    ...
    Example:
        >>> play_catchphrase('image.jpg', 4)
        >>> play_catchphrase('image.jpg') - three pieces will be revealed.
